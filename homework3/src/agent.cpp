...
int main(int argc, char* argv[]){
	...
	if(argc != 8){
		fprintf(stderr,"usage: %s <sender IP> <agent IP> <recv IP> <sender port> <agent port> <recv port> <loss_rate>\n", argv[0]);
		fprintf(stderr, "ex: ./agent 127.0.0.1 127.0.0.1 127.0.0.1 8887 8888 8889 0.3\n");
		exit(1);
	} else {
		setIP(ip[0], argv[1]);
		setIP(ip[1], argv[2]);
	...
	while(1){
		...
		if(segment_size > 0){
			...
			if(strcmp(ipfrom, ip[0]) == 0 && portfrom == port[0]) {
				...
				if(s_tmp.head.ack) {
					fprintf(stderr, "get sender's ack segment\n");
					exit(1);
				}
				total_data++;
				if(s_tmp.head.fin == 1) {
				...
				if(s_tmp.head.ack == 0) {
					fprintf(stderr, "get receiver's non-ack segment\n");
					exit(1);
				}
				if(s_tmp.head.fin == 1) {
					printf("get		finack\n");
